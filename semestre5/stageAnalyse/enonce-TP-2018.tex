...
\title{Stage Analyse pour l'ingénieur Ensimag 1A \\
  Mini-stage Scilab/Latex}
\author{Brigitte Bidegaray \and Samah Bouzidi \and Simon Pageaud \and Valérie Perrier}
\date{Novembre 2018}
...
partie 2, le polycopié et le fichier exemple fourni enonce-TP-2018.tex. Enfin, faites les
...
\end{itemize}
\begin{center}
\textbf{Date limite de rendu: 3 décembre 2018}\\
\end{center}
...
Scilab est un logiciel de calcul \emph{numérique}, l'essentiel des objets que l'on manipule sont des matrices de nombres.
...
\begin{verbatim}
$ emacs enonce-TP-2018.tex &
\end{verbatim}
\item Pour compiler ce fichier, taper la commande :
\begin{verbatim}
$ pdflatex enonce-TP-2018.tex
\end{verbatim}
\item Si la compilation s'est exécutée sans erreurs, un fichier
  ``enonce-TP-2018.pdf'' a été créé, à visualiser par exemple avec:
\begin{verbatim}
$ evince enonce-TP-2018.pdf &
\end{verbatim}
\end{enumerate}
{\it NB: Il est conseillé de garder la fenêtre avec le pdf active (en
insérant un $\&$ à la fin de la commande). Après chaque modification
...
Il existe deux principales manière d'inclure du contenu mathématique dans un document Latex : au milieu de texte, en indiquant encadrant le contenu mathématique par des balises \$, ou alors sur une ligne séparée avec l'environnement {\tt equation}.
...
vous pouvez récupérer le source du TP (le .tex) dans le répertoire de Chamilo.


\subsection{Sensibilisation à l'arithmétique machine }

Scilab, comme la plupart des langages de calcul, utilise la norme
...
\begin{exo} On cherche à calculer la valeur de l'intégrale
  ...
  \item Donner une formule de récurrence pour calculer cette intégrale, puis évaluer $I_{20}$
    à partir de $I_0$
  \item Evaluer $I_{20}$ en utilisant le développement en série de $e^x$.
  \item Conclusions ?
  \end{enumerate}
\end{exo}

\begin{exo} Ici, on va calculer directement une approximation de
  $I_{20}$ en utilisant la méthode des rectangles.  Ecrire une fonction
...
\subsection{Etude du phénomène de Gibbs}
...
\end{equation}
où les coefficients $c_{n}(f)$ de $f$ sont appelés \emph{coefficients de Fourier}
...
\begin{equation}
	f(t) = a_{0}(f) + \sum_{n=1}^{\infty} \left( a_{n}(f)\cos\left(2\pi\frac{n}{T}t\right) + b_{n}(f)\sin\left(2\pi\frac{n}{T}t\right) \right),
\end{equation}
...
\begin{exo} {\it (pratique)} Ecrire un programme Scilab traçant la
...
\begin{exo} Dans cet exercice, nous nous intéressons au théorème suivant :\\
  ...
  \item Ecrire un programme Scilab permettant de visualiser les disques
